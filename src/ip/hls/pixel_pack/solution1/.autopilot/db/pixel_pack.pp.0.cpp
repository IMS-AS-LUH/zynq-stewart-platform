...
# 2 "<built-in>" 2
# 1 "pixel_pack/pixel_pack.cpp" 2




# 1 "pixel_pack/pixel_pack.hpp" 1
...
# 6 "pixel_pack/pixel_pack.hpp" 2
# 1 "C:/Xilinx/VITIS_~1/2022.1/common/technology/autopilot\\ap_int.h" 1
# 7 "pixel_pack/pixel_pack.hpp" 2
...
__attribute__((sdx_kernel("pixel_pack", 0))) void pixel_pack(narrow_stream& stream_in_24, wide_stream& stream_out_32,
                int mode, ap_uint<8> alpha);
# 6 "pixel_pack/pixel_pack.cpp" 2

__attribute__((sdx_kernel("pixel_pack", 0))) void pixel_pack(narrow_stream& stream_in_24, wide_stream& stream_out_32,
    int mode, ap_uint<8> alpha) {
#line 12 "C:/Users/Sebastian/Documents/Studium/Master/HwP/PYNQ/boards/ip/hls/pixel_pack/script.tcl"
#pragma HLSDIRECTIVE TOP name=pixel_pack
# 8 "pixel_pack/pixel_pack.cpp"
...
#pragma HLS INTERFACE axis depth=24 port=stream_out_32 register
 ...
 switch (mode) {
 case 0:
  VITIS_LOOP_21_1: while (!delayed_last) {
#pragma HLS pipeline II=4
 delayed_last = last;
   ap_uint<96> buffer;
   ap_uint<4> has_last;
   ap_uint<4> has_user;
   VITIS_LOOP_27_2: for (int j = 0; j < 4; ++j) {
    if (!last) {
   ...
   if (!delayed_last) {
    VITIS_LOOP_37_3: for (int i = 0; i < 3; ++i) {
...
#pragma HLS pipeline II=1
 ap_uint<32> data;
  ...
  break;
 case 2:
  VITIS_LOOP_62_5: while (!delayed_last) {
#pragma HLS pipeline II=4
 delayed_last = last;
   bool user = false;
   ap_uint<32> data;
   VITIS_LOOP_67_6: for (int i = 0; i < 4; ++i) {
    if (!last) {
     stream_in_24.read(in_pixel);
     user |= in_pixel.user;
     last = in_pixel.last;
  ...
  break;
 case 3:
  VITIS_LOOP_84_7: while (!last) {
#pragma HLS pipeline II=2
 bool user = false;
   ap_uint<32> data;
   VITIS_LOOP_88_8: for (int i = 0; i < 2; ++i) {
    stream_in_24.read(in_pixel);
    user |= in_pixel.user;
    last = in_pixel.last;
  ...
  break;
 case 4:
  VITIS_LOOP_101_9: while (!last) {
#pragma HLS pipeline II=2
 bool user = false;
   ap_uint<48> data;
   VITIS_LOOP_105_10: for (int i = 0; i < 2; ++i) {
    stream_in_24.read(in_pixel);
    user |= in_pixel.user;
    last = in_pixel.last;
