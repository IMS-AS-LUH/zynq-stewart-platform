...
architecture behave of trace_cntrl_64_trace_cntrl_s_axi is
    ...
    constant ADDR_TRIGGER_DATA_0  : INTEGER := 16#10#;
    constant ADDR_TRIGGER_DATA_1  : INTEGER := 16#14#;
    constant ADDR_TRIGGER_CTRL    : INTEGER := 16#18#;
    ...
    signal int_trigger         : UNSIGNED(63 downto 0) := (others => '0');
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_TRIGGER_DATA_0 =>
                        rdata_data <= RESIZE(int_trigger(31 downto 0), 32);
                    when ADDR_TRIGGER_DATA_1 =>
                        rdata_data <= RESIZE(int_trigger(63 downto 32), 32);
...
-- ----------------------- Register logic ----------------
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_TRIGGER_DATA_0) then
                    int_trigger(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_trigger(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_TRIGGER_DATA_1) then
                    int_trigger(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_trigger(63 downto 32));
                end if;
            end if;
        end if;
    end process;
